...
def _read_hdf5_attr(attr):
    ...
    """
    # If passed an actual h5py Attribute object, get dtype from it
    if hasattr(attr, 'dtype'):
        dtype = attr.dtype
        val = attr[()]
    else:
        # Already the value — infer type from numpy array or Python native
        val = attr
...
class HdfIO2:
    ...
    def _init(self):
        ...
        if "imageid_max" in images_grp.attrs:
            self._nimg = int(images_grp.attrs["imageid_max"]) + 1
        else:
            self._nimg = 0
    ...
    def write_header(self, d, index=-1):
        ...
        if index < 0:
            current_max = int(images_grp.attrs["imageid_max"])
            index = current_max + 1
